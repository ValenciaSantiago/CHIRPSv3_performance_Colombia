...
categorical_ind_function_v2 <- function(n_file,
                                        var_name,
                                        dir_scale,    # work directory
                                        file_name,    # file names
                                        na_threshold,  # maximum number of allow missing values
  ...
  for (p in 1:length(low_pcp_threshold)){
    ...
    #////////////////////////////////////////////////////////////////////////////////////////////
    #metrics
    # the Hansen–Kuipers discriminant (HK)
    # HK = POD - POFD
    ...
    pcp_file_ii$Fv2 <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv2_cat == 1, 1, 0)
    pcp_file_ii$Cv2 <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv2_cat == 0, 1, 0)
    
    pcp_file_ii$Fv3_era5 <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv3_era5_cat == 1, 1, 0)
    pcp_file_ii$Cv3_era5 <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv3_era5_cat == 0, 1, 0)
    
    pcp_file_ii$Fv3_imerg <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv3_imerg_cat == 1, 1, 0)
    pcp_file_ii$Cv3_imerg <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv3_imerg_cat == 0, 1, 0)
    
    POFD_v2 <- sum(pcp_file_ii$Fv2) / (sum(pcp_file_ii$Fv2) + sum(pcp_file_ii$Cv2))
    POFD_v3_era5 <- sum(pcp_file_ii$Fv3_era5) / (sum(pcp_file_ii$Fv3_era5) + sum(pcp_file_ii$Cv3_era5))
    POFD_v3_imerg <- sum(pcp_file_ii$Fv3_imerg) / (sum(pcp_file_ii$Fv3_imerg) + sum(pcp_file_ii$Cv3_imerg))
    
    HK_v2       <- POD_v2 - POFD_v2
    HK_v3_era5  <- POD_v3_era5 - POFD_v3_era5
    HK_v3_imerg <- POD_v3_imerg - POFD_v3_imerg
    ...
    results <- data.frame(
      ...
      fBIASv3_imerg = fBIASv3_imerg,
      HK_v2 = HK_v2,
      HK_v3_era5 = HK_v3_era5,
      HK_v3_imerg = HK_v3_imerg
    )
    colnames(results) <- paste0(colnames(results),"_",p_levels[p])
    #results
...
performance_cat_daily <- lapply(seq(1, length(ideam_gauges_metadata[, 1])), 
                                  function(n_file) {
                                    categorical_ind_function_v2(n_file,
                                                             "pcp_ideam_flag",
                                                              dir_results_daily,
                                                              "DAILY",
                                                              1,"2001","2023")})
...
fwrite(performance_cat_daily_df,paste0("G:/My Drive/R4C_et_al/4_IDEAM_GPPs/" ,
                                "/res_performance_daily_categorical_indices_2001_2023_df_v2.csv"))
...
fwrite(performance_cat_daily_df,paste0("G:/My Drive/R4C_et_al/4_IDEAM_GPPs/" ,
                  "/res_performance_daily_categorical_indices_2001_2023_df_scale_v2.csv"))
...
fwrite(performance_cat_daily_df,paste0("G:/My Drive/R4C_et_al/4_IDEAM_GPPs/" ,
      "/res_performance_daily_categorical_indices_2001_2023_dry_season_df_v2.csv"))
...
fwrite(performance_cat_daily_df,paste0("G:/My Drive/R4C_et_al/4_IDEAM_GPPs/" ,
     "/res_performance_daily_categorical_indices_2001_2023_wet_season_df_v2.csv"))
...
categorical_ind_function_pentad <- function(n_file,
  ...
  for (p in 1:length(low_pcp_threshold)){
    ...
    # Probability of False Detection (POFD)
    pcp_file_ii$Fv2  <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv2_cat == 1, 1, 0)
    pcp_file_ii$Fv2s <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpv2_cat == 1, 1, 0)
    pcp_file_ii$Cv2  <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv2_cat == 0, 1, 0)
    pcp_file_ii$Cv2s <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpv2_cat == 0, 1, 0)
    
    
    pcp_file_ii$Fv3  <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv3_cat == 1, 1, 0)
    pcp_file_ii$Fv3s <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpv3_cat == 1, 1, 0)
    pcp_file_ii$Cv3  <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpsv3_cat == 0, 1, 0)
    pcp_file_ii$Cv3s <- ifelse(pcp_file_ii$ideam_cat == 0 & pcp_file_ii$chirpv3_cat == 0, 1, 0)
    
    
    POFD_v2  <- sum(pcp_file_ii$Fv2) / (sum(pcp_file_ii$Fv2) + sum(pcp_file_ii$Cv2))
    POFD_v3  <- sum(pcp_file_ii$Fv3) / (sum(pcp_file_ii$Fv3) + sum(pcp_file_ii$Cv3))
    POFD_v2s <- sum(pcp_file_ii$Fv2s) / (sum(pcp_file_ii$Fv2s) + sum(pcp_file_ii$Cv2s))
    POFD_v3s <- sum(pcp_file_ii$Fv3s) / (sum(pcp_file_ii$Fv3s) + sum(pcp_file_ii$Cv3s))
    
    
    HK_v2  <- POD_v2 - POFD_v2
    HK_v3  <- POD_v3 - POFD_v3
    HK_v2s <- POD_v2s - POFD_v2s
    HK_v3s <- POD_v3s - POFD_v3s
